...
def get_now(city):
    ...
    response2[-1] += kb.slash
    response2[-1] += "👉 " + "<b>" + str(desc.strip()) + "</b>" + kb.nl
    response2[-1] += kb.slash
    ...
    response2[-1] += kb.slash
    response2[-1] += (f"🌇 <b>Восход</b> {sunset}   ⤼   🌄 <b>Закат</b> {sunrise}{kb.nl} ")
...
def get_today(city):
    try:
        url = "https://www.gismeteo.ru" + json_load[city]
    except:
        url = ''
    response = requests.get(url, headers={
        "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36"
    ...
    all_times = [i.text for i in all_wraps.find(class_="widget-row widget-row-time").find_all(class_="row-item")]
    ...
    for el in all_wraps.find(class_="widget-row widget-row-humidity row-with-caption").find_all(
        ...
        try:
            ...
            humidity.append("-")
    for i in range(8):
        response2.append("<b>" + (all_times[i]) + "</b>" + "\n")
        response2[-1] += kb.slash
        response2[-1] += (
